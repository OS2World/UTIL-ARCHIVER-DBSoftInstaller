...
int main(int argc, char *argv[])
{
   ...
   unsigned char tempbuffer[1024], hexbuffer[10];
   int charsread=0, z, k, n;

   printf("Include- version 2.0 (c) 1999 DBSoft\n\n");
   if(argc!=2)
      {
         printf("Usage: Include <filename.cfg>\n");
         exit(1);
      }
   if((cfgfile = fopen(argv[1], "rt"))==NULL)
      {
   ...
   fclose(cfgfile);
   fprintf(installfile, "%s-%s-%s-%s-%s-%s-%s-%s-%s-%d-%d-%d",
            INSTALLER_APPLICATION, INSTALLER_VERSION, INSTALLER_TITLE,
            INSTALLER_PATH, INSTALLER_FOLDER, INSTALLER_PROGRAM, 
            INSTALLER_SHADOW, INSTALLER_SETS, INSTALLER_SYSVAR,
            INSTALLER_BITMAP_WIDTH, INSTALLER_BITMAP_HEIGHT,
            INSTALLER_PACKAGE_COUNT);
